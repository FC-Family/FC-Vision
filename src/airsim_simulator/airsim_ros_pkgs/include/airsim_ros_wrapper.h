 ...
 * Description  :    This file is the header file of airsim simulator core functions.
...
public:
    ...
    ros::AsyncSpinner traj_async_spinner_;
    bool is_used_lidar_timer_cb_queue_;
    bool is_used_img_timer_cb_queue_;
    bool is_used_traj_timer_cb_queue_;
...
private:    
    ...
    void traj_viz_timer_cb(const ros::TimerEvent& event);
    ...
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr imageToPointCloud(const sensor_msgs::ImagePtr& depth_msg, const sensor_msgs::ImagePtr& rgb_msg, const CameraIntrinsics& intrinsics, float depth_scale = 0.001f, float max_depth = 20.0f);
    ros::Publisher world_img_rgb_pcd_pub_;
    ...
    void update_traj_visualization(const std::string& vehicle_name, const msr::airlib::Vector3r& position, const ros::Time& vehicle_time);
    void plot_traj_visualization(const std::string& vehicle_name, const ros::Time& now);
    ...
    nav_msgs::Odometry get_lidar_odom_from_airsim(const msr::airlib::LidarData& lidar_data, const std::string& vehicle_name) const;
    sensor_msgs::PointCloud2 get_world_lidar_msg_from_airsim(const sensor_msgs::PointCloud2& local_lidar_msg, const nav_msgs::Odometry& lidar_odom) const;
...
private:
    ...
    ros::Publisher gimbal_quat_pub_;
    ...
    msr::airlib::RpcLibClientBase airsim_client_traj_;

    ros::NodeHandle nh_;
    ros::NodeHandle nh_private_;

    /// LiDAR
    Eigen::Vector3d lidar_d2body_t_vector_;
    Eigen::Matrix3d lidar_d2body_R_matrix_; 

    /// Camera
    Eigen::Vector3d cam_d2body_t_vector_;
    Eigen::Matrix3d cam_d2body_R_matrix_;
    float cam_max_depth_;
    std::vector<std::vector<Eigen::Vector3d>> scale_direction_table_;
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr rgb_cloud_, transformed_cloud_;
    int skip_pixels_;
    ...
    ros::CallbackQueue traj_timer_cb_queue_;

    std::mutex drone_control_mutex_;
    std::mutex traj_viz_mutex_;
    ...
    std::unordered_map<std::string, std::deque<TrajStamped>> traj_points_map_;
    std::unordered_map<std::string, ros::Time> last_traj_pub_time_;
    std::vector<float> traj_color_rgba_{1.0f, 0.0f, 0.0f, 1.0f};
    double traj_thickness_ = 1.0;
    double traj_duration_ = 3.0;
    double traj_window_sec_ = 0.0;
    size_t traj_max_points_ = 150;
    double traj_publish_interval_sec_ = 0.1;
    int traj_buckets_ = 4;
    double traj_alpha_pow_ = 1.5;
    double traj_min_alpha_ = 0.05;
    double traj_min_size_ = 1.0;
    double traj_min_spacing_ = 0.0;
    double traj_max_segment_ = 0.0;
    bool enable_traj_viz_ = true;
    ...
    ros::Timer airsim_traj_viz_timer_;
    ...
    // camera info
    double cam_offset_x_ = 0.0, cam_offset_y_ = 0.0, cam_offset_z_ = 0.0; 
