...
)
{
	...
	*/
	...
	tf::Quaternion tf_quat(desired_attitude.x(), desired_attitude.y(), desired_attitude.z(), desired_attitude.w());
    ...
    u.roll_rate = roll; // angle
	u.pitch_rate = pitch; // angle
	...
	double limit_angle = 30.0*3.14/180.0;
	if(std::abs(u.roll_rate)>=limit_angle)
		ROS_INFO("ROLL ANGLE LIMIT!");
	if(std::abs(u.pitch_rate)>=limit_angle)
		ROS_INFO("PITCH ANGLE LIMIT!");

	uav_utils::limit_range(u.roll_rate,limit_angle);
	uav_utils::limit_range(u.pitch_rate,limit_angle);
