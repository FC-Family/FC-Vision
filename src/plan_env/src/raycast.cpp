...
double intbound(double s, double ds) {
  ...
  if (ds < 0.0) { s = -s; ds = -ds; }
  s = std::fmod(std::fmod(s, 1.0) + 1.0, 1.0);
  return (1.0 - s) / ds;
}

double safe_val(double v) {
...
bool RayCaster::input(const Eigen::Vector3d& start, const Eigen::Vector3d& end) {
  start_ = (start - origin_) / resolution_;
  end_ = (end - origin_) / resolution_;

  x_ = (int)std::floor(start_.x());
  y_ = (int)std::floor(start_.y());
  z_ = (int)std::floor(start_.z());
  startX_ = (int)std::floor(start_.z());
  startY_ = (int)std::floor(start_.y());
  startZ_ = (int)std::floor(start_.z());
  endX_ = (int)std::floor(end_.x());
  endY_ = (int)std::floor(end_.y());
  endZ_ = (int)std::floor(end_.z());
  direction_ = (end_ - start_);

  auto prep = [](double s, double d, int& step, double& tMax, double& tDelta) 
  {
    if (d == 0.0) 
    {
      step = 0;
      tMax = std::numeric_limits<double>::infinity();
      tDelta = std::numeric_limits<double>::infinity();
    } 
    else 
    {
      step = (d > 0.0) ? 1 : -1;
      tMax = intbound(s, d);
      tDelta = std::abs(1.0 / d);
    }
  };
  prep(start_.x(), direction_.x(), stepX_, tMaxX_, tDeltaX_);
  prep(start_.y(), direction_.y(), stepY_, tMaxY_, tDeltaY_);
  prep(start_.z(), direction_.z(), stepZ_, tMaxZ_, tDeltaZ_);

  step_num_ = 0;
  max_steps_ = std::abs(endX_ - x_) + std::abs(endY_ - y_) + std::abs(endZ_ - z_) + 3;
  stop_flag_ = false;
...
bool RayCaster::nextId(Eigen::Vector3i& idx) {
  idx = Eigen::Vector3i(x_, y_, z_);

  if (stop_flag_) return false;

  const double eps = 1e-6;
  const double next_t = std::min(tMaxX_, std::min(tMaxY_, tMaxZ_));
  if (next_t > 1.0 + eps) stop_flag_ = true;

  step_num_++;
  if (step_num_ > max_steps_) 
  {
    std::cout << "start_: " << startX_ << ", " << startY_ << ", " << startZ_ << std::endl;
    std::cout << "end_: " << endX_ << ", " << endY_ << ", " << endZ_ << std::endl;
    std::cout << "current_: " << x_ << ", " << y_ << ", " << z_ << std::endl;
    std::cerr << "Warning: Too many steps, exiting loop!" << std::endl; // Warn the loop to avoid infinite steps
    return false;
  }

  const double aX = safe_val(tMaxX_);
  const double aY = safe_val(tMaxY_);
  const double aZ = safe_val(tMaxZ_);
  ...
  if (aX <= aY) 
  {
    if (aX <= aZ) 
    {
      if (stepX_ == 0) return false;
      x_ += stepX_;
      tMaxX_ += tDeltaX_;
    } 
    else 
    {
      if (stepZ_ == 0) return false;
      z_ += stepZ_;
      tMaxZ_ += tDeltaZ_;
    }
  } 
